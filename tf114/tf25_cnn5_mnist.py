...
# Layer1
w1 = tf.compat.v1.get_variable('w1', shape=[2, 2, 1, 128], initializer=tf.keras.initializers.GlorotUniform()) #kernel 사이즈, 채널, output
...
# Layer4 DNN
w4 = tf.compat.v1.get_variable('w4', shape=[6*6*32, 100], initializer=tf.keras.initializers.GlorotUniform())
b4 = tf.compat.v1.Variable(tf.zeros([100]), name = 'b4')
L4 = tf.nn.relu(tf.matmul(L_flat, w4) + b4)
L4 = tf.nn.dropout(L4, rate= 0.3)
 
# Layer5 DNN
w5 = tf.compat.v1.get_variable('w5', shape=[100, 10], initializer=tf.keras.initializers.GlorotUniform())
...
predictions = batch_predict(x_test, batch_size=500)
