...
#데이터 분류
x_train, x_test, y_train, y_test = train_test_split(x, one_hot_y, train_size=0.8, random_state=123456, stratify=one_hot_y)
print(np.unique(y_test, return_counts=True))
#(array([False,  True]), array([9900, 1650], dtype=int64))

#모델 생성
model = Sequential()
model.add(Dense(256, input_dim = 12))
model.add(Dense(128))
...
history = model.fit(x_train, y_train, epochs=120, batch_size=100, verbose= 1, validation_split=0.2, callbacks=[es])
